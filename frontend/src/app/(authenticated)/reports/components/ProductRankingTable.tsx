    ...
    <div className="flex flex-col gap-4 bg-surface border border-border/80 p-5 rounded-2xl shadow-md">
      ...
      >
        {products.map((p: any, idx: number) => {
          ...
          return (
            <tr key={p.productId} className="border-b border-border/50 hover:bg-surface-secondary/20 transition">
              <td className="px-6 py-4 text-sm font-extrabold text-text-muted">#{rank}</td>
              <td className="px-6 py-4 text-sm font-bold text-text-primary">{p.productName}</td>
              <td className="px-6 py-4 text-sm font-medium text-text-secondary font-mono">{p.sku}</td>
              <td className="px-6 py-4 text-sm text-text-muted font-bold uppercase tracking-wide">
                {p.category}
              </td>
