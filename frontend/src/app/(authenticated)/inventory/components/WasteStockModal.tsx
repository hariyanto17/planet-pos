...
export const WasteStockModal: React.FC<Props> = ({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrorMsg(err?.data?.message || "Failed to record waste");
    }
  };

  return (
    <Modal isOpen={isOpen} onClose={onClose} title="Record Waste">
      <form onSubmit={handleSubmit} className="flex flex-col gap-4">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <Input
            ...
            placeholder="e.g. 5 or 0.25"
            required
          />
        </div>

        <div className="flex flex-col gap-1.5">
          <label className="text-zinc-400 text-xs font-bold uppercase tracking-wider">Wastage Reason</label>
          <Input
            type="text"
            value={remarks}
            onChange={(e) => setRemarks(e.target.value)}
            placeholder="Spoiled, expired, spilt, etc."
            required
          />
        </div>
